...
if ($_POST['SaveSection'] == true) {
	$thename = $_POST['name'];
	file_put_contents("inf/$thename.html", $_POST['mce_0']);
}
$fields = array('red');
function ss($name, $section, $type, $default) {
	global $fields;
	$file = "inf/$name.html";
	if (!file_exists($file)) {
		if ($default) {
...
<div class="prut8Eje" style="width:100%; height: auto;">
...
function editbutton() {
	if ($_SESSION['ss_loggedin'] == 'true') {
		echo "<div style='position: fixed; top:0px; right: 0px; background: white;'><p><a href='?p=a'>Edit</a></p></div>";
	}
}
